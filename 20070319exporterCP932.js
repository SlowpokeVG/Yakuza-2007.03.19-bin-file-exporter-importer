...
for (let currentParam = 0; currentParam < parameterAmount; currentParam++) {
    let parameterName = shiftjis.decode(binFile.subarray(16 + 64 * currentParam, 16 + 64 * currentParam + 48)).replace(/\0(?![^\0])/g, '');
    ...
    switch (parameterType) {
        ...
        case 0: case 3: case 8:
            let strings = shiftjis.decode(parameterBuffer).split('\0');
            for (let currentEntry = 0; currentEntry < entryAmount; currentEntry++) {
        ...
        case 1: case 4:
            let stringsForTable = shiftjis.decode(parameterBuffer).split('\0', parameterCount);
            let tableStart = 0;
            let wordCount = 0;
        ...
        case 2: case 5:
            ...
            while ((currentWord < parameterCount) && (currentByte < parameterBuffer.length)) {
                ...
                jsonObject[entryID][parameterName] = shiftjis.decode(parameterBuffer.subarray(currentByte, wordEndByte));
